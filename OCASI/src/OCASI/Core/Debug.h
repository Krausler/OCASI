...
#define OCASI_ASSERT(x) if(!(x)) { OCASI_DEBUGBREAK(); OCASI_LOG_ERROR("Assertion failed at line {0} in file {1}.", __LINE__, Path(__FILE__).string()); }
#define OCASI_ASSERT_MSG(x, msg) if(!(x)) { OCASI_DEBUGBREAK(); OCASI_LOG_ERROR("Assertion failed: {}", msg); }
